...
rrweb provides an fflate-based simple compress function in [@browser-replay/packer](../../packages/packer/).

You can use it by passing it as the `packFn` in the recording.

```js
import { pack } from '@browser-replay/packer';
...
```js
import { unpack } from '@browser-replay/packer';
