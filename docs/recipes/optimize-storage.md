...
rrweb provides an fflate-based simple compress function in [@dom-replay/packer](../../packages/packer/).

You can use it by passing it as the `packFn` in the recording.

```js
import { pack } from '@dom-replay/packer';
...
```js
import { unpack } from '@dom-replay/packer';
