...
function ToggleSwitch({
  ...
  return (
    <div className={`br-switch${disabled ? ' disabled' : ''}`}>
      <input
        type="checkbox"
        id={id}
        checked={checked}
      ...
      />
      <label htmlFor={id} />
      <span className="br-switch__label">{label}</span>
    </div>
  );
}
...
function renderInactiveMarker(marker: InactivePeriodMarker, idx: number) {
  return (
    <div
      ...
      title={marker.name}
      className="br-progress__inactive"
      style={{
        width: marker.width,
...
export function Controller({
  ...
  return (
    <div className="br-controller">
      <div className="br-timeline">
        <span
          className="br-timeline__time"
          aria-live="polite"
          aria-atomic="true"
        >
        ...
        <div
          ref={progressRef}
          className={`br-progress${skipping ? ' disabled' : ''}`}
          role="slider"
          tabIndex={0}
        ...
        >
          <div className="br-progress__step" style={{ width: percentage }} />
          {state.inactivePeriods.map(renderInactiveMarker)}
          {state.customEvents.map(renderCustomMarker)}
          <div className="br-progress__handler" style={{ left: percentage }} />
        </div>
        <span className="br-timeline__time">{formatTime(totalTime)}</span>
      </div>

      <div className="br-controller__btns">
        <button
          ref={playButtonRef}
