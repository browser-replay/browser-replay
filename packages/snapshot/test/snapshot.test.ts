...
describe('isBlockedElement()', () => {
  const subject = (html: string, opt: any = {}) =>
    _isBlockedElement(render(html), 'br-block', opt.blockSelector);
  ...
  it('blocks prohibited className', () => {
    expect(subject('<div class="foo br-block bar" />')).toEqual(true);
  });

  it('does not block random data selector', () => {
    expect(subject('<div data-br-block />')).toEqual(false);
  });

  it('blocks blocked selector', () => {
    expect(
      subject('<div data-br-block />', { blockSelector: '[data-br-block]' }),
    ).toEqual(true);
  });
});
...
describe('style elements', () => {
  ...
  it('should serialize all rules on stylesheets with mix of insertion type', () => {
    ...
    expect(normalizeCssText(serialized.attributes._cssText)).toBe(
      normalizeCssText(
        'section.working {color: pink;}body {color: red;}/* br_split */section {color: blue;}',
      ),
    );
  });
});
